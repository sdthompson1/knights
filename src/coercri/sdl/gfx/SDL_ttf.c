...
/* Modifications by Stephen Thompson:

   23-Mar-2008: Instruct freetype NOT to use the patented TrueType
     hinting techniques, even if they are available. This is because
     the auto-hinter actually gives better results than the TrueType
     native hinting (in my opinion).

   4-May-2008: Fix potential memory leak in TTF_OpenFontIndexRW (the
   RWops is not closed in some cases when the function returns NULL).
