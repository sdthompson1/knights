...
# Enable online platform support
ONLINE_PLATFORM_FLAGS = -DONLINE_PLATFORM -DONLINE_PLATFORM_DUMMY -DUSE_VM_LOBBY

CPPFLAGS = -DUSE_FONTCONFIG -DDATA_DIR=$(DATA_DIR) -DLUA_INCLUDES_REQUIRE_EXTERN_C $(ONLINE_PLATFORM_FLAGS)
CFLAGS = -g
CXXFLAGS = $(CFLAGS)
...
src/misc/config_map.o: src/misc/config_map.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) `pkg-config libcurl --cflags` `pkg-config lua-c++ --cflags` `pkg-config libenet --cflags` -Isrc -I.  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
...
src/misc/round.o: src/misc/round.cpp
	...
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
src/online_platform/dummy_online_platform.o: src/online_platform/dummy_online_platform.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) `pkg-config libcurl --cflags` `pkg-config lua-c++ --cflags` `pkg-config libenet --cflags` -Isrc/coercri -Isrc/misc  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
src/online_platform/dummy_platform_lobby.o: src/online_platform/dummy_platform_lobby.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) `pkg-config libcurl --cflags` `pkg-config lua-c++ --cflags` `pkg-config libenet --cflags` -Isrc/coercri -Isrc/misc  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
...
src/rstream/rstream.o: src/rstream/rstream.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) `pkg-config libcurl --cflags` `pkg-config lua-c++ --cflags` `pkg-config libenet --cflags`  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
...
src/svr_main/server_main.o: src/svr_main/server_main.cpp
	...
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
src/virtual_server/knights_vm.o: src/virtual_server/knights_vm.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) `pkg-config libcurl --cflags` `pkg-config lua-c++ --cflags` `pkg-config libenet --cflags` -Isrc/coercri -Isrc/rstream  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
src/virtual_server/risc_vm.o: src/virtual_server/risc_vm.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) `pkg-config libcurl --cflags` `pkg-config lua-c++ --cflags` `pkg-config libenet --cflags` -Isrc/coercri -Isrc/rstream  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
src/virtual_server/tick_data.o: src/virtual_server/tick_data.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) `pkg-config libcurl --cflags` `pkg-config lua-c++ --cflags` `pkg-config libenet --cflags` -Isrc/coercri -Isrc/rstream  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
...
$(KNIGHTS_BINARY_NAME): $(OFILES_MAIN)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -o $@ $^ `pkg-config sdl2 --libs` `pkg-config freetype2 --libs` `pkg-config libcurl --libs` `pkg-config lua-c++ --libs` `pkg-config libenet --libs` -lfontconfig -lX11 $(BOOST_LIBS)


$(SERVER_BINARY_NAME): $(OFILES_SERVER)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -o $@ $^ `pkg-config libcurl --libs` `pkg-config lua-c++ --libs` `pkg-config libenet --libs` $(BOOST_LIBS)
