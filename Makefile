...
src/svr_main/config.o: src/svr_main/config.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) $(LUA_CFLAGS) `pkg-config libenet --cflags` -Isrc/misc -Isrc/coercri -Isrc/engine -Isrc/external -Isrc/rstream -Isrc/server -Isrc/shared  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
...
$(SERVER_BINARY_NAME): $(OFILES_SERVER)
	$(CXX) $(LDFLAGS) -o $@ $^ $(LUA_LIBS) `pkg-config libenet --libs` $(BOOST_LIBS)
