...
src/external/lua/ltm.o: src/external/lua/ltm.c
	...
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
	  rm -f $*.d
src/external/lua/lua.o: src/external/lua/lua.c
	$(CC) $(CPPFLAGS) $(CFLAGS) -Isrc/external/lua  -MD -c -o $@ $<
	@cp $*.d $*.P; \
	  sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	      -e '/^$$/ d' -e 's/$$/ :/' < $*.d >> $*.P; \
...
install_knights: $(KNIGHTS_BINARY_NAME)
	...
	$(INSTALL) -m 644 knights_data/tiles.lua $(DATA_DIR)
...
install_docs:
	...
	$(INSTALL) -m 644 docs/svn-commit.tmp~ $(DOC_DIR)
